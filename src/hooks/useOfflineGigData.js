...
async function fetchGigData(gigId) {
  const [
    ...
    supabase
      .from('gig_lineup')
      .select(
        'id, profile_id, placeholder_id, confirmed, instrument_id, travel_cost_pence, vocal_role, is_captain, profiles(full_name, phone, share_phone_on_daysheet), instruments(name), placeholder_musicians(name)'
      )
      .eq('gig_id', gigId),
