...
async function fetchGigData(gigId) {
  const [
    ...
    supabase
      .from('gigs')
      .select(
        '*, venues(name, address, latitude, longitude), clients(name), bands(name), songs:first_dance_song_id(title, artist)'
      )
      .eq('id', gigId)
      .single(),
    ...
    supabase
      .from('gig_lineup')
      .select(
        'id, profile_id, placeholder_id, confirmed, instrument_id, travel_cost_pence, vocal_role, is_captain, is_dj, is_roadie, fee_pence, confirmed_fee_pence, profiles(full_name, phone, share_phone_on_daysheet), instruments(name), placeholder_musicians(name)'
      )
      .eq('gig_id', gigId),
